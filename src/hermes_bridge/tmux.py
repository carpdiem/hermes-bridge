...
def _create_session_command(agent: AgentConfig, name_expr: str, remote_command: str) -> str:
    ...
    return (
        ...
        f"{tmux} set-option -t \"$name\" remain-on-exit on >/dev/null 2>&1 || true; "
        "deadline=$(( $(date +%s) + 8 )); "
        "while :; do "
        f"if ! {tmux} has-session -t \"$name\" >/dev/null 2>&1; then "
        "printf 'tmux session exited during startup: %s\\n' \"$name\" >&2; exit 90; "
        "fi; "
        f"dead=$({tmux} list-panes -t \"$name\" -F '#{{pane_dead}}' 2>/dev/null | grep -c '^1$' || true); "
        f"total=$({tmux} list-panes -t \"$name\" -F '#{{pane_dead}}' 2>/dev/null | wc -l | tr -d ' '); "
        "if [ \"${total:-0}\" != 0 ] && [ \"$dead\" = \"$total\" ]; then "
        "printf 'tmux session command exited during startup: %s\\n' \"$name\" >&2; "
        f"{tmux} capture-pane -p -S -120 -t \"$name\" >&2 || true; "
        "exit 90; "
        "fi; "
        "if [ $(date +%s) -ge $deadline ]; then break; fi; "
        "sleep 1; "
        "done; "
        f"{tmux} set-option -t \"$name\" remain-on-exit off >/dev/null 2>&1 || true; "
        f"{style}; "
        "printf '%s\\n' \"$name\""
    )
